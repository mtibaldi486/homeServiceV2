...
CREATE TABLE IF NOT EXISTS `db_paris`.`prestation` (
  ...
  INDEX `fk_prestation_categorie1_idx` (`categorie_nom` ASC) VISIBLE,
  CONSTRAINT `fk_prestation_categorie1`
    FOREIGN KEY (`categorie_nom`)
    REFERENCES `db_paris`.`categorie` (`nom`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `db_paris`.`reservation` (
  ...
  PRIMARY KEY (`id_reservation`, `user_id_user`, `user_ville_reference`),
  INDEX `fk_reservation_prestation1_idx` (`prestation_id_prestation` ASC) VISIBLE,
